        ...
        <GlassCard className="flex items-center justify-between p-3 bg-white/5 border-b border-white/10 sticky top-0 z-50 backdrop-blur-xl mb-6 overflow-visible">
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-white/5 text-slate-300 hover:text-white hover:bg-white/10 rounded-lg transition-colors text-xs font-bold border border-white/5"
                >
                ...
                <button
                    onClick={resetStudentAttempt}
                    className="flex items-center gap-2 px-3 py-1.5 bg-neon-blue/20 text-neon-blue hover:bg-neon-blue hover:text-white rounded-lg transition-colors text-xs font-bold"
                >
    ...
    const ProfileMenu = () => (
        <div className="relative z-50">
            <button
                ...
                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 border border-white/10 rounded-full pl-2 pr-4 py-1.5 transition-all backdrop-blur-md"
            >
            ...
            <AnimatePresence>
                {isProfileOpen && (
                    <motion.div
                        ...
                        className="absolute right-0 top-14 w-80 bg-[#0a0a0a]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
                    >
                                ...
                                >
                                    <div className="flex flex-col">
                                        <span className="text-xs font-bold text-slate-300 group-hover:text-white flex items-center gap-2">
                                            <Calendar className="w-3 h-3 opacity-50" /> {new Date(h.date).toLocaleDateString()}
                                        </span>
                                        <span className="text-[10px] text-slate-500">{new Date(h.date).toLocaleTimeString()}</span>
                                    </div>
                                    <div className={`px-2 py-1 rounded-md text-xs font-bold bg-white/5 ${h.score >= 80 ? 'text-neon-green' : h.score >= 50 ? 'text-neon-yellow' : 'text-neon-pink'}`}>
                                        {h.score}%
                                    </div>
                                </button>
                            )) : (
                        ...
                        <div className="p-2 border-t border-white/10 bg-black/20">
                            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 text-xs font-bold text-red-400 hover:text-red-300 hover:bg-red-500/10 py-2 rounded-lg transition-colors">
                ...
                {payload.map((entry, index) => {
                    ...
                    return (
                        <div key={`item-${index}`} className={`flex items-center gap-2 cursor-pointer transition-all duration-300 ${isDimmed ? 'opacity-30 blur-[1px]' : 'opacity-100 scale-105'}`} onClick={() => handleLegendClick(entry.value)}>
                            ...
                            <span className="text-xs font-bold uppercase text-slate-300 hover:text-white">
                                {entry.value} {isFocused && <Eye className="w-3 h-3 inline ml-1 text-white" />}
                            </span>
                        </div>
                    );
                })}
                {focusMode !== 'all' && (
                    <button onClick={() => setFocusMode('all')} className="flex items-center gap-1.5 px-3 py-1 bg-white/10 hover:bg-white/20 rounded-full text-[10px] font-bold uppercase text-slate-300 transition-colors ml-4">
                                    ...
                                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                        <GlassCard className="border-l-4 border-neon-blue relative">
                                            <div className="flex justify-between items-center mb-6">
                                                <div>
                                                    <h3 className="text-neon-blue font-bold uppercase tracking-widest text-xs mb-1">Teacher's Reference</h3>
                                                    <h2 className="text-2xl font-serif text-white">Lesson Captured</h2>
                                                </div>
                                                <div className="flex items-center gap-6">
                                                    <div className="text-right">
                                                        <span className="block text-[10px] text-slate-400 uppercase tracking-widest font-bold">Notes</span>
                                                        <span className="text-xl font-bold text-white">{teacherData.notes ? teacherData.notes.length : 0}</span>
                                                    </div>
                                                    {teacherAudioURL && <AudioPlayerButton ref={teacherAudioRef} audioUrl={teacherAudioURL} colorClass="text-neon-blue" />}
                                                </div>
                                            </div>
                                            <div className="bg-white rounded-xl p-4 shadow-inner min-h-[150px] flex items-center justify-center">
                                                <SheetMusic xmlData={teacherData.musicxml} />
                                            </div>
                                        </GlassCard>
                                    </motion.div>
                                )}
                                    ...
                                    <div className="flex flex-col h-full">
                                        {teacherData ? (
                                            <GlassCard className="flex-1 flex flex-col border-t-4 border-neon-blue bg-white/5">
                                                <div className="flex justify-between items-center mb-6">
                                                    <h3 className="text-neon-blue font-bold uppercase tracking-widest text-xs">Target Lesson</h3>
                                                    {teacherAudioURL && <AudioPlayerButton ref={teacherAudioRef} audioUrl={teacherAudioURL} label="Listen" colorClass="text-neon-blue" />}
                                                </div>
                                                <div className="flex-1 bg-white rounded-xl p-4 flex items-center justify-center shadow-inner overflow-hidden">
                                            ...
                                            <GlassCard className="flex-1 flex flex-col items-center justify-center border-dashed border-2 border-white/10 bg-transparent opacity-50">
                                                <School className="w-12 h-12 text-slate-600 mb-4" />
                                                <p className="text-slate-500 text-sm">No Teacher Lesson Found</p>
                                            </GlassCard>
                                        )}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <GlassCard className="border-t-4 border-neon-blue">
                                            <div className="flex justify-between items-center mb-4">
                                                <h3 className="font-bold text-neon-blue flex items-center gap-2 text-xs uppercase tracking-wider"><School className="w-4 h-4" /> Reference</h3>
                                                {teacherAudioURL && <AudioPlayerButton ref={teacherAudioRef} audioUrl={teacherAudioURL} colorClass="text-neon-blue" />}
                                            </div>
                                            <div className="bg-white rounded-xl overflow-hidden p-2 min-h-[180px] flex items-center justify-center shadow-inner">
                                                {teacherData ? <SheetMusic xmlData={teacherData.musicxml} /> : <span className="text-black/30 text-sm">No data</span>}
                                            </div>
                                        </GlassCard>
                                        <GlassCard className="border-t-4 border-neon-pink">
                                            <div className="flex justify-between items-center mb-4">
                                                <h3 className="font-bold text-neon-pink flex items-center gap-2 text-xs uppercase tracking-wider"><GraduationCap className="w-4 h-4" /> Your Transcription</h3>
                                                <div className="flex items-baseline gap-1">
                                                    <span className={`text-4xl font-serif font-bold ${getScoreColor(studentData.feedback.score)}`}>{studentData.feedback.score}</span>
                                                    <span className="text-sm text-slate-500 font-serif">%</span>
                                                </div>
                                            </div>
                                            <div className="bg-white rounded-xl overflow-hidden p-2 min-h-[180px] flex items-center justify-center shadow-inner">
                                                <SheetMusic xmlData={studentData.musicxml} />
                                            </div>
                                        </GlassCard>
                                    </div>
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        <GlassCard className="lg:col-span-1 max-h-[500px] overflow-y-auto custom-scrollbar">
                                            <h3 className="font-bold text-white mb-4 flex items-center gap-2 sticky top-0 bg-[#0a0a0a] z-10 py-2">
                                                <Zap className="w-4 h-4 text-neon-yellow" /> Note Analysis
                                            </h3>
                                                ...
                                                {studentData.feedback.detailed_breakdown?.map((item, i) => (
                                                    <div key={i} className={`flex flex-col p-3 rounded-lg text-sm border border-white/5 ${item.status === 'match' ? 'bg-green-500/10 text-green-200' : 'bg-red-500/10 text-red-200'}`}>
                                                        <div className="flex justify-between items-center mb-1">
                                                            <span className="font-bold text-xs opacity-70">Note {item.index}</span>
                                                            {item.status === 'match' ? <CheckCircle className="w-3 h-3 text-green-400" /> : <XCircle className="w-3 h-3 text-red-400" />}
                                                        </div>
                                        ...
                                        <div className="lg:col-span-2 space-y-6">
                                            ...
                                            <div className="bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-xl text-black h-[250px] relative">
                                                <div className="flex justify-between mb-2">
                                                    <h4 className="text-xs font-bold uppercase tracking-widest text-slate-500">Dynamics (Waveform)</h4>
                                                    <div className={`w-2 h-2 rounded-full ${isScrubbingEnabled ? 'bg-neon-green' : 'bg-slate-300'}`} />
                                                </div>
                                                <ResponsiveContainer width="100%" height="85%">
